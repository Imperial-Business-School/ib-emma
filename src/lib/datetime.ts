...
// deadline is deemed to have passed: 10:00 UK time on the same calendar
// date. Returns null on malformed input.
//
// Used everywhere overdue detection compares against "now": a deadline
// picked as e.g. 2026-06-30 is passed once local UK time ticks past
// 2026-06-30T10:00.
export function endOfDeadlineDay(dateStr: string | null | undefined): Date | null {
  ...
  if (!m) return null;
  const [, y, mo, d] = m;
  return parseUkLocalDateTime(`${y}-${mo}-${d}T10:00`);
}
